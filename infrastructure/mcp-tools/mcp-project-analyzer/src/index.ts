...
async function cloneRepositories(args: any) {
...
async function analyzeRepository(repoPath: string, includeFiles: string[]) {
  const repoAnalysis: any = {
    type: 'unknown',
    technologies: [],
    hasDockerfile: false,
    hasKubernetes: false,
...
async function scanDockerConfigs(args: any) {
...
async function analyzeCICDPipelines(args: any) {
  const { projectPath } = args;
  const pipelines: any[] = [];
...
async function identifyAWSRequirements(args: any) {
  const { projectPath } = args;
...
async function generateArchitectureReport(args: any) {
